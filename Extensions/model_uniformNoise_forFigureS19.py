...
class Model:
    def __init__(self, params, track_times, noise_type, **kwargs):
        """
        Create model by initialising an array of agent opinions 
    ...
    def run(self, t_simulation):
        ...
        # remove agent monologues (agents communicating with themselves)
...
def init2G(n, a0=0.07432029, mu0=0.34018943, sig0=0.25329727, a1=0.10308521, mu1=0.77908659, sig1=0.13464983):
    ...
    the values are fitted to obtain a distribution that resembles the distribution of the six Americas (Maibach 2011). 
    """
...
def main(noise_type, expname, n, ic, mu_arr, seeds, track_times, resolution, verbose=False, **kwargs):
    ...
    for _, eps in enumerate(eps_arr):
        ...
        if len(seeds)==1: 
            fname = f"data/model-UNIFORM{noise_type}_"+expname+"{}Initial_eps{:.3f}_seeds{}.ncdf".format(ic,  eps, seeds[0])
        else:
            fname = f"data/model-UNIFORM{noise_type}_"+expname+"{}Initial_eps{:.3f}_seeds{}-{}.ncdf".format(ic,  eps, seeds[0], seeds[-1])
        print("saved: ", fname)
        d.to_netcdf(fname)
    if verbose: print("finished")

    if len(seeds)==1:
        ncdfs = [xr.open_dataset(f"data/model-UNIFORM{noise_type}_{expname}{ic}Initial_eps{eps:.3f}_seeds{s}.ncdf", engine="netcdf4") for eps in eps_arr]
        ncdfs_merged = xr.merge(ncdfs)
        ncdfs_merged.attrs["initial_dist"] = params["initial_dist"]
        ncdfs_merged.to_netcdf(f"data/model-UNIFORM{noise_type}_{expname}{ic}Initial_seeds{s}.ncdf")
    return 
...
if __name__=="__main__":
    ...
    # CHOOSE PARAMETERS 
    track_times = np.arange(0, int(1e5)+1, step=1000)
    mu_arr = [0.5]
    n = 100
    seeds = list(range(10))    # list(range(1000))
    resolution = "SA"
    ic = "2G-6AM"
    noise_type = "ambiguityNoise"   # in ["ambiguityNoise", "selectionNoise", "exogenousNoise", "adaptationNoise"]
